...
BEGIN
    ...
    -- 注意：使用表别名明确指定列来源，避免列名歧义
    SELECT cp.id, cp.customer_phone
...
-- 验证函数已创建
COMMENT ON FUNCTION assign_customer_to_employee(VARCHAR) IS '分配客户给员工，使用随机顺序分配（ORDER BY RANDOM()），已修复列名歧义问题';
